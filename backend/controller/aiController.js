...
const generateOutline = async ( req, res ) => {
    try{
        ...
        //find and extract the json rray from the response text
        const startIndex = text.indexOf("[");
        const endIndex = text.lastIndexOf("]");

        if(startIndex === -1 || endIndex === -1){
            console.error("Could not find JSON array in I response: ", text);
            return res
                .status(500)
        ...
        try{
            ...
            res.status(500).json({message: "Failed to parse AI response, invalid JSON"});
        }
    }
    catch(error){
